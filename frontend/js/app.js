...
let temporarySearch;
function closeLocalSearch() { if(!temporarySearch)return;canvas.elements.get(temporarySearch.dataset.panelId)?.classList.remove("search-active");temporarySearch.remove();temporarySearch=undefined; }
function showResults(panel, results, query) {
  ...
  for(const item of results){const button=document.createElement("button");button.textContent=item.name;button.onclick=async()=>{const owner=[...canvas.nodes.values()].find(n=>(n.files||[]).some(x=>x.id===item.id)||(n.folders||[]).some(x=>x.id===item.id));if(owner)canvas.revealPanel(owner.panelInstanceId,item.id);else await canvas.openSearchResult(panel.panelInstanceId,item);closeLocalSearch();};card.append(button);}
}
canvas.actions.localSearch = (panelId) => { closeLocalSearch();const panel=canvas.nodes.get(panelId),panelElement=canvas.elements.get(panelId);if(!panel||!panelElement)return;const stack=document.createElement("section");stack.className="local-search-stack";stack.dataset.panelId=panelId;stack.style.width=`${panel.renderedWidth}px`;stack.style.transform=`translate(${panel.x}px,${panel.y-36}px) translateY(-100%)`;const card=document.createElement("div");card.className="temporary-search";card.hidden=true;const input=document.createElement("input");input.className="header-search";input.type="search";input.placeholder="Search recursively…";stack.append(card,input);canvas.world.append(stack);panelElement.classList.add("search-active");temporarySearch=stack;input.focus();let timer;
  input.oninput=()=>{clearTimeout(timer);const q=input.value.trim();if(q.length<2){card.hidden=true;card.replaceChildren();return;}timer=setTimeout(async()=>{try{const response=await searchFolder(panel.folderId,q);showResults(panel,response.results,q);status.textContent=`${response.results.length} 件${response.truncated?"（検索上限）":""}`;}catch(error){showError(error);}},300);};input.onkeydown=e=>{if(e.key==="Escape"){closeLocalSearch();canvas.canvas.focus();}}; };
