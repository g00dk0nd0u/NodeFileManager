...
export class FolderCanvas {
  ...
  layoutFamily(parentId) { const parent=this.nodes.get(parentId),children=[...this.nodes.values()].filter(node=>node.visualParentPanelId===parentId).sort((a,b)=>a.x-b.x); if(!parent||!children.length)return;
    if(children.length===1){this.moveBranchTo(children[0].panelInstanceId,parent.x+300,parent.y);return;} const total=children.length*260+(children.length-1)*20,start=parent.x+130-total/2,y=parent.y+(parent.renderedHeight||220)+70; children.forEach((child,index)=>this.moveBranchTo(child.panelInstanceId,start+index*280,y)); }
  ...
  async reconcileMovedFolder(oldFolderId,movedItem) { const roots=[...this.nodes.values()].filter(node=>node.folderId===oldFolderId);if(!roots.length)return;const parentResponse=await this.actions.getParent?.(movedItem.id),parentMetadata=parentResponse?.parent||null;
    for(const root of roots){Object.assign(root,{id:movedItem.id,folderId:movedItem.id,name:movedItem.name,path:movedItem.path,fsParentFolderId:movedItem.parentId});const normalParent=this.panelForFolder(movedItem.parentId,root.workingSetId);if(normalParent){root.visualParentPanelId=normalParent.panelInstanceId;delete root.compactParent;}else{root.visualParentPanelId=null;root.compactParent=parentMetadata;}await this.reconcileVisibleDescendants(root);}this.render();this.changed();}
  ...
  continueDrag(event){const s=this.dragSession;if(!s||s.pointerId!==event.pointerId)return;const dx=event.clientX-s.startX,dy=event.clientY-s.startY;if(!s.dragging&&Math.hypot(dx,dy)<5)return;if(!s.dragging&&s.type==="panel")this.selectFolder(s.nodeId);s.dragging=true;for(const[id,o]of s.origins){const n=this.nodes.get(id);if(n){n.x=o.x+dx/this.viewport.zoom;n.y=o.y+dy/this.viewport.zoom;}}if(s.type==="panel")this.canvas.dataset.dragIntent=this.dragIntent(s,event.clientX,event.clientY)?.type||"return";this.updatePositions();this.renderSets();this.renderEdges();}
