...
export class FolderCanvas {
  constructor(element, onChange, loadChildren, history = null) {
    ...
    this.onChange = onChange; this.loadChildren = loadChildren; this.history = history; this.actions = {}; this.dragSession = null;
  ...
  async addRoot(folder) {
    const before=this.captureWorkspaceState();
    ...
    this.nodes.set(panel.panelInstanceId, panel); try{await this.loadNode(panel);}catch(error){this.applyWorkspaceState(before);throw error;} this.selected = panel.panelInstanceId; this.render(); this.changed();this.commitWorkspaceEdit("Add root Folder Panel",before);
  }
  ...
  async openChild(parentPanelId, folder) {
    ...
    this.nodes.set(child.panelInstanceId, child);try{await this.loadNode(child);}catch(error){this.applyWorkspaceState(before);throw error;} this.reflowHierarchy(parentPanelId); this.render(); this.changed();this.commitWorkspaceEdit("Open child Folder Panel",before); this.actions.visit?.(folder.id);
  }
  async openParent(childId) { const child=this.nodes.get(childId), folder=child?.compactParent; if(!child||!folder)return;const before=this.captureWorkspaceState(); const existing=this.panelForFolder(folder.id,child.workingSetId); if(existing){child.visualParentPanelId=existing.panelInstanceId;delete child.compactParent;this.reflowHierarchy(existing.panelInstanceId);this.render();this.changed();this.commitWorkspaceEdit("Open Compact Parent",before);return;}
    const parent={...folder,id:folder.id,folderId:folder.id,panelInstanceId:uid("panel"),workingSetId:child.workingSetId,visualParentPanelId:null,fsParentFolderId:null,x:child.x,y:child.y,childrenLoaded:false,revealFolderId:child.folderId};child.visualParentPanelId=parent.panelInstanceId;delete child.compactParent;this.nodes.set(parent.panelInstanceId,parent);try{await this.loadNode(parent);}catch(error){this.applyWorkspaceState(before);throw error;}parent.x=child.x-panelWidth(parent)-BRANCH_SPACING.trail;this.layoutFamily(parent.panelInstanceId);this.render();this.changed();this.commitWorkspaceEdit("Open Compact Parent",before); }
  ...
  isolate(panelId) { const root = this.nodes.get(panelId),parent=this.nodes.get(root?.visualParentPanelId); if (!root || !parent) return false;const before=this.captureWorkspaceState(); const setId = this.newSet(); root.compactParent={id:parent.folderId,name:parent.name,path:parent.path,kind:"folder"};root.fsParentFolderId=parent.folderId;root.visualParentPanelId = null; for (const node of [root, ...this.descendants(panelId)]) node.workingSetId = setId; this.reflowHierarchy(parent.panelInstanceId);this.reflowHierarchy(root.panelInstanceId);this.cleanupSets(); this.render(); this.changed();this.commitWorkspaceEdit("Isolate",before);return true; }
  reattach(panelId, destinationSetId = null) { const root = this.nodes.get(panelId); if (!root || root.visualParentPanelId || !root.fsParentFolderId) return false;
    ...
    for (const node of [root, ...this.descendants(panelId)]) node.workingSetId = parent.workingSetId; this.reflowHierarchy(parent.panelInstanceId); this.workingSets.delete(old); this.render(); this.changed();this.commitWorkspaceEdit("Reattach",before); return true; }
  ...
  closeNode(panelId) {if(!this.nodes.has(panelId))return false;const before=this.captureWorkspaceState(); const parentId=this.nodes.get(panelId)?.visualParentPanelId;this.removeBranch(panelId,false);if(parentId)this.reflowHierarchy(parentId);this.cleanupSets(); this.clearSelection(false); this.render(); this.changed();this.commitWorkspaceEdit("Close Folder Panel branch",before);return true; }
  ...
  cancelDrag(event){if(this.dragSession?.pointerId===event.pointerId)this.finishDrag(false);} finishDrag(save){const s=this.dragSession;this.clearFilesystemFeedback();delete this.canvas.dataset.dragIntent;if(!s)return;this.dragSession=null;if(s.element.hasPointerCapture?.(s.pointerId))s.element.releasePointerCapture(s.pointerId);if(save&&s.dragging){this.changed();if(s.type==="set")this.commitWorkspaceEdit("Move Working Set",s.historyBefore);}else if(s.dragging){for(const[id,o]of s.origins){const n=this.nodes.get(id);if(n)Object.assign(n,o);}this.updatePositions();this.renderSets();this.renderEdges();}}
  ...
  captureWorkspaceState(){return{nodes:[...this.nodes].map(([id,node])=>[id,structuredClone(Object.fromEntries(Object.entries(node).filter(([key])=>key!=="preview")))]),workingSets:structuredClone([...(this.workingSets||new Map())])};}
  applyWorkspaceState(state){const previews=new Map([...this.nodes].flatMap(([id,node])=>node.preview?[[id,node.preview]]:[]));this.nodes=new Map(structuredClone(state.nodes));for(const[id,preview]of previews)if(this.nodes.has(id))this.nodes.get(id).preview=preview;this.workingSets=new Map(structuredClone(state.workingSets));this.selected=null;this.selectedItem=null;this.render();this.changed();}
  commitWorkspaceEdit(label,before){if(!this.history)return;const after=this.captureWorkspaceState();this.history.record({label,undo:async()=>this.applyWorkspaceState(before),redo:async()=>this.applyWorkspaceState(after)});}
