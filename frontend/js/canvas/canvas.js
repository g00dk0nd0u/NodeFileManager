...
export class FolderCanvas {
  ...
  renderEdges() { this.edges.replaceChildren();const canvasRect=this.canvas.getBoundingClientRect();for(const child of this.nodes.values()){const parent=this.nodes.get(child.visualParentPanelId);if(!parent)continue;const parentElement=this.elements.get(parent.panelInstanceId),childElement=this.elements.get(child.panelInstanceId),sourceRow=parentElement?.querySelector(`.folder-item[data-id="${CSS.escape(child.folderId)}"]`),header=childElement?.querySelector(".node-title");if(!sourceRow||!header)continue;const rowRect=sourceRow.getBoundingClientRect(),headerRect=header.getBoundingClientRect(),right=child.x>=parent.x+(parent.renderedWidth||0)/2;const from=right?{x:rowRect.right,y:rowRect.top+rowRect.height/2}:{x:rowRect.left+rowRect.width/2,y:rowRect.bottom},to=right?{x:headerRect.left,y:headerRect.top+headerRect.height/2}:{x:headerRect.left+headerRect.width/2,y:headerRect.top};const source=this.screenToWorld(from.x-canvasRect.left,from.y-canvasRect.top),target=this.screenToWorld(to.x-canvasRect.left,to.y-canvasRect.top),exit=right?{x:source.x+18,y:source.y}:{x:source.x,y:source.y+18},approach=right?{x:target.x-18,y:target.y}:{x:target.x,y:target.y-18},mx=(exit.x+approach.x)/2,my=(exit.y+approach.y)/2;const path=document.createElementNS("http://www.w3.org/2000/svg","path");path.setAttribute("d",`M ${source.x} ${source.y} L ${exit.x} ${exit.y} C ${right?mx:exit.x} ${right?exit.y:my}, ${right?mx:approach.x} ${right?approach.y:my}, ${approach.x} ${approach.y} L ${target.x} ${target.y}`);this.edges.append(path);}}
