...
let renameTarget=null;const renameDialog=document.querySelector("#rename-dialog");canvas.actions.rename=()=>{renameTarget=canvas.selectedItem||canvas.nodes.get(canvas.selected);if(!renameTarget)return;document.querySelector("#rename-name").value=renameTarget.name;renameDialog.showModal();document.querySelector("#rename-name").select();};document.querySelector("#rename-confirm").addEventListener("click",async event=>{event.preventDefault();const item=renameTarget,name=document.querySelector("#rename-name").value;if(!item||name===item.name){renameDialog.close("cancel");return;}try{const result=await renameItem(item.id,name);let currentId=result.item.id;await canvas.applyRename(item.id,result.item);history.record({label:`Rename ${item.name}`,undo:async()=>{const replay=await replayFileOperation(result.item.operationToken,"undo");currentId=await reconcileFilesystemReplay(replay,currentId,"rename",item.parentId,item.parentId);},redo:async()=>{const replay=await replayFileOperation(result.item.operationToken,"redo");currentId=await reconcileFilesystemReplay(replay,currentId,"rename",item.parentId,item.parentId);}});canvas.selectedItem=null;renameDialog.close("confirm");renderNavigation(await getNavigation());if(item.parentId)await canvas.refresh(item.parentId);}catch(error){showError(error);}});renameDialog.addEventListener("close",()=>{renameTarget=null;});
canvas.actions.transfer = async (id, destinationId, copy = false, kind = "file") => { try { const movedPanel=kind==="folder"?[...canvas.nodes.values()].find(node=>node.folderId===id):null, sourceId = canvas.selectedItem?.parentId || canvas.nodes.get(movedPanel?.visualParentPanelId)?.folderId || movedPanel?.fsParentFolderId; const result=await (copy ? copyItem : moveItem)(id, destinationId);if(copy)history.clear();if(!copy&&kind==="folder")await canvas.reconcileMovedFolder(id,result.item);if(!copy){let currentId=result.item.id;history.record({label:`Move ${result.item.name}`,undo:async()=>{const replay=await replayFileOperation(result.item.operationToken,"undo");currentId=await reconcileFilesystemReplay(replay,currentId,"move",sourceId,destinationId);},redo:async()=>{const replay=await replayFileOperation(result.item.operationToken,"redo");currentId=await reconcileFilesystemReplay(replay,currentId,"move",sourceId,destinationId);}});}canvas.clearSelection(); if (!copy) renderNavigation(await getNavigation()); if (sourceId) await canvas.refresh(sourceId); if (destinationId !== sourceId) await canvas.refresh(destinationId); status.textContent = copy ? "コピーしました" : "移動しました"; } catch (error) { showError(error); } };
