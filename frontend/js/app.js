...
function showResults(panel, results, query) {
  ...
  for(const item of results){const button=document.createElement("button");button.textContent=item.name;button.onclick=async()=>{const owner=[...canvas.nodes.values()].find(n=>(n.files||[]).some(x=>x.id===item.id)||(n.folders||[]).some(x=>x.id===item.id));if(owner)canvas.revealPanel(owner.panelInstanceId,item.id);else await canvas.openSearchResult(panel.panelInstanceId,item);closeLocalSearch();};card.append(button);}updateLocalSearchHeight(stack);
}
