...
async function reconcileCreation(_result,_id,parentId){await canvas.refresh(parentId);renderNavigation(await getNavigation());}
...
document.querySelector("#new-folder-confirm").addEventListener("click", async (event) => {
  event.preventDefault(); const name = document.querySelector("#new-folder-name").value,parentId=newFolderParent;
  try { const response=await createFolder(parentId, name),result={item:response.folder};const entry=recordFilesystemOperation(history,{label:`Create Folder ${name}`,token:result.item.operationToken,initialId:result.item.id,replay:replayFileOperation,reconcile:(replay,currentId)=>reconcileCreation(replay,currentId,parentId),reportError:showError});newFolderDialog.close("confirm");await entry.reconcileInitial(result);status.textContent = `フォルダーを作成しました: ${name}`; } catch (error) { showError(error); }
});
